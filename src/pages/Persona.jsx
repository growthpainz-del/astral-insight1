...
const STANDARD_PERSONA_TEMPLATE = `Persona: The Glitch-Aware Upgrade Seeker
...
export default function PersonaPage() {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      toast.success("Persona saved. Future AI readings will use this voice.");
    } catch (e) {
  ...
  const handleSaveToDeck = async () => {
    ...
    setSaving(true);
    try {
      await base44.entities.Deck.update(selectedDeck.id, {
        ai_reading_coach: preamble.trim(),
      });
      toast.success(`Persona saved to "${selectedDeck.name}".`);
    } catch (e) {
      toast.error("Failed to save to deck. Please try again.");
    } finally {
      setSaving(false);
    }
  };
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-950 via-indigo-950 to-purple-950 text-white p-6">
      <div className="max-w-2xl mx-auto">
          ...
          <CardContent className="space-y-4">
            ...
            <div>
              ...
              <p className="text-xs text-white/60 mt-2">
                This description steers the AI's voice in your readings. Keep
                it brief but specific.
              </p>
            </div>
            ...
            <div className="flex justify-between gap-2 flex-wrap">
              <Button
                variant="ghost"
                onClick={() => setPreamble(STANDARD_PERSONA_TEMPLATE)}
              ...
              <div className="flex gap-2 ml-auto">
                <Button
                  ...
                  disabled={saving || !preamble.trim()}
                  className="border-white/20 text-white hover:bg-white/10"
                >
                  {saving
                    ? "Saving…"
                    : selectedDeck
                    ? `Save to "${selectedDeck.name}"`
                    : "Save to Deck"}
                </Button>
                <Button
                  onClick={handleSave}
        ...
        <div className="mt-6 text-sm text-white/70">
          Tip: You can delete any mistakenly created decks in Dashboard → Data → Deck.
        </div>
