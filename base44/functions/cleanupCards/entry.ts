...
Deno.serve(async (req) => {
  try {
    ...
    // Fetch all Cards
    const limit = 500;
    const cards = await base44.entities.Card.list('-created_date', limit);
    for (const card of cards) {
      if (card.spirit_wheel_icon_url && SYMBOL_MAP[card.spirit_wheel_icon_url]) {
        await base44.entities.Card.update(card.id, {
          spirit_wheel_icon_url: SYMBOL_MAP[card.spirit_wheel_icon_url]
        });
        modifiedCount++;
      }
    }

    return Response.json({ success: true, message: `Fixed ${modifiedCount} Cards in the database.` });
  } catch (error) {
